...
class Topic(models.Model):
    ...
    image = models.ImageField(default='', upload_to='uploads/forum/covers/', verbose_name=_("image"))
    append_file = models.FileField(default='', upload_to='uploads/forum/files/', verbose_name=_("append_file"))
    append_video = models.FileField(default='', upload_to='uploads/forum/files/', verbose_name=_("append_video"))
    content_raw = models.TextField(max_length=2000, blank=True, verbose_name=_("raw content"))
    ...
    def save(self, *args, **kwargs):
        ...
        mentioned_users = []
        self.content_raw = BeautifulSoup(self.content_rendered).get_text()
    ...
    class Meta:
        ...
        verbose_name = '帖子' # 单数时显示内容
        verbose_name_plural = '帖子'  # 复数时显示内容
...
class Node(models.Model):
    title = models.CharField(max_length=30, verbose_name=_("类别"))
    description = models.TextField(default='', blank=True, verbose_name=_("对该类别的描述"))

    def __str__(self):
        return self.title

    class Meta:
        verbose_name = '类别' # 单数时显示内容
        verbose_name_plural = '类别'  # 复数时显示内容
...
class Announcement(models.Model):
    user = models.ForeignKey(USER_MODEL, related_name='announcements', verbose_name=_("user"))
    title = models.CharField(max_length=64, verbose_name=_("title"))
    content_raw = models.TextField(max_length=2000, blank=True, verbose_name=_("raw content"))
    content_rendered = models.TextField(max_length=3000, blank=True, verbose_name=_("rendered content"))
    pub_date = models.DateTimeField(auto_now_add=True, db_index=True, verbose_name=_("published time"))
    view_count = models.IntegerField(default=0, verbose_name=_("view count"))
    ...
    def increase_view_count(self):
        Announcement.objects.filter(pk=self.id).update(view_count=F('view_count') + 1)
    ...
    class Meta:
        verbose_name = '公告' # 单数时显示内容
        verbose_name_plural = '公告'  # 复数时显示内容


@python_2_unicode_compatible
class EmailCode(models.Model):
    email = models.CharField(max_length=64, verbose_name=_("email"))
    code = models.IntegerField(default=0, verbose_name=_("code"))
    status = models.IntegerField(default=0, verbose_name=_("status"))

    def __str__(self):
        return '%s: %d' % (self.email, self.code)
